...
function edahabia_chargily_config()
{
	return array(
		'FriendlyName' => array(
			'Type' => 'System',
			'Value' => 'EDAHABIA card payment',
		) ,
		'apiKey' => array(
...
function edahabia_chargily_link($params)
{
	$webhook_url = $params['systemurl'] . '/modules/gateways/callback/edahabia_chargily.php';
	...
	if (curl_errno($ch))
	{
		...
		logTransaction("edahabia_chargily", array_merge($ch_error, $params) , "Error on Payment Request", $params);
	}
	curl_close($ch);
	$data = json_decode($result, true);
	if (empty($data["checkout_url"]))
	{
		logTransaction("edahabia_chargily", array_merge($data, $params) , "Error on Payment Request", $params);
